...
const QueryMaster = ({ level, onComplete, onExit, playerHero }) => {
    ...
    const handleSelectedClick = (block, index) => {
        if (status === 'WON') return;
    ...
    return (
        <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-4">
            ...
            {/* Status Message */}
            {message && (
                <div className={`px-6 py-3 rounded-full font-bold flex items-center gap-2 animate-bounce mb-6
            ${status === 'WON' ? 'bg-green-500 text-black' : status === 'ERROR' ? 'bg-red-500 text-white' : 'bg-yellow-500 text-black'}
          `}>
