...
export default function ServerGuardian({ level, onComplete, onExit, playerHero }: ServerGuardianProps) {
        ...
        prev.forEach((p) => {
          ...
          if (target) {
            ...
            if (Math.abs(newX - target.x) < 2) {
              setEnemies((es) =>
                es.map((e) => (e.id === target.id ? { ...e, hp: e.hp - 35 } : e)).filter((e) => e.hp > 0)
              )
            } else {
              next.push({ ...p, x: newX })
            }
  ...
  const handleComplete = async () => {
    ...
    await saveGameSession({
      ...
      levelId: level.id,
      score: health * 100 + timeLeft,
      duration,
      won: true,
      xpEarned: level.xpReward,
      gameConfig: {
        towersPlaced: towers.length,
        enemiesKilled: 0, // Could track this
      },
    })
